  ...
  .then(jason => {
    ...
    document.getElementById('value').addEventListener('click', () => {
      ...
      filterjason = jason.filter(profile => {
        if ((profile.promo === promo || promo === 'Promo') && (profile.campus === campus || campus === 'Campus')) { return true }
        return false
      })

      if (searchBar != []) {
        filterjason = filterjason.filter(profile => {
          if ((searchBar.includes(profile.firstName)) || (searchBar.includes(profile.lastName))) { return true }
          return false
        })
      }
